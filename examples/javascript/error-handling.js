/**
 * Sun Position API - Error Handling Example
 *
 * This example demonstrates proper error handling when using the Sun Position API.
 ...
 */
async function callSunPositionAPIWithErrorHandling(queryParams = {}) {
  try {
...
async function callWithRetry(maxRetries = 3, initialDelay = 1000) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const result = await callSunPositionAPIWithErrorHandling({
