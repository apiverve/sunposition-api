...
class SunpositionAPIClientError(Exception):
...
class ValidationError(SunpositionAPIClientError):
...
class SunpositionAPIClient:
    ...
    VALIDATION_RULES = {"lat": {"type": "number", "required": True, "min": -90, "max": 90}, "lon": {"type": "number", "required": True, "min": -180, "max": 180}, "date": {"type": "string", "required": False, "format": "date", "default": "Today"}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the SunpositionAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises SunpositionAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise SunpositionAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[SunpositionAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns SunPositionData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises SunpositionAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise SunpositionAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise SunpositionAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise SunpositionAPIClientError(f"Request failed: {str(e)}")
